...
COMPLIANCE_RULES: list[_ComplianceRule] = [
    ...
    _ComplianceRule(
        ...
        patterns=[
            ...
            r"一定.*能", r"完美", r"无.*风险",
            r"最负责任", r"史上.*最好", r"绝对.*安全",
            r"零风险", r"完全.*无忧",
        ],
        severity="YELLOW",
        suggestion="修改为相对表述，如「领先」「优质」「重要」等非绝对化用语。",
    ),
    _ComplianceRule(
        name="虚假比较",
        description="不当对比竞品",
...
1. 共情开场：理解客户的顾虑或现状
2. 真实案例：用身边或行业案例说明风险
3. 自然过渡：从案例引出保险保障的必要性
4. 产品介绍：用通俗语言说明产品核心价值
5. 行动引导：邀请进一步了解或预约面谈
...
def build_script_prompt(style: str, customer_context: dict, product_info: str = "") -> str:
    ...
    if customer_context.get("stage"):
        stage_name = {
            ...
            "closing": "促成签约",
            "follow_up": "售后跟进",
        }.get(customer_context["stage"], customer_context["stage"])
        ctx_parts.append(f"销售阶段：{stage_name}")
    ...
    if customer_context.get("insurance_knowledge"):
        ctx_parts.append(f"保险认知水平：{customer_context['insurance_knowledge']}")
...
3. 话术开头直接称呼客户（使用客户姓名）
4. 最后要自然引导到下一步行动（如预约面谈、提供方案等）
5. 使用口语化但专业的中文
6. 可以适当提及华安保险的品牌优势，但不得贬低其他公司"""
